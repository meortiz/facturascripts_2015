 ...
 */
class tab_automocion_vehiculo_pre extends fs_controller
{
   public $allow_delete;
   public $marca;
   public $modelo;
   ...
   protected function private_core()
   {
      if(in_array('presupuestos_y_pedidos',$GLOBALS['plugins']))
      {
         ...
         $this->relacion = array();
         $this->documento = new presupuesto_cliente();
         ...
         foreach($this->vehiculo_documento->get_presupuestos_from_vehiculo($this->id_vehiculo) as $vehiculo) {
            $this->vehiculos[$vehiculo->idpresupuesto] = $this->vehiculo->get($vehiculo->idvehiculo);
            $this->relacion[$vehiculo->idpresupuesto] = array($vehiculo->idvehiculo, $vehiculo->kilometraje);
            $this->documentos[$vehiculo->idpresupuesto] = $this->documento->get($vehiculo->idpresupuesto);
         }

         $this->total = count($this->vehiculo_documento->get_presupuestos_from_vehiculo($this->id_vehiculo));
      }
   }
