...
/**
 * Description of tab_automocion_vehiculo_pedp
 *
 * @author Francesc Pineda Segarra
 */
class tab_automocion_vehiculo_pedp extends fs_controller
{
   public $allow_delete;
   public $marca;
   public $modelo;
   ...
   protected function private_core()
   {
      if(in_array('presupuestos_y_pedidos',$GLOBALS['plugins']))
      {
         ...
         $this->relacion = array();
         $this->documento = new pedido_proveedor();
         ...
         foreach($this->vehiculo_documento_prov->get_pedidos_from_vehiculo($this->id_vehiculo) as $vehiculo) {
            $this->vehiculos[$vehiculo->idpedido] = $this->vehiculo->get($vehiculo->idvehiculo);
            $this->relacion[$vehiculo->idpedido] = array($vehiculo->idvehiculo, $vehiculo->kilometraje);
            $this->documentos[$vehiculo->idpedido] = $this->documento->get($vehiculo->idpedido);
         }

         $this->total = count($this->vehiculo_documento_prov->get_pedidos_from_vehiculo($this->id_vehiculo));
      }
   }
