...
/**
 * El modelo vehiculo_documento_prov relaciona los vehiculos con sus documentos en 
 * la tabla automocion_vehiculo_documentosp.
 *
 * @author Francesc Pineda Segarra
 */

class vehiculo_documento_prov extends fs_model
{
   public $id;
   public $idvehiculo;
   public $idpedido;
   ...
   {
      parent::__construct('automocion_vehiculo_documento_prov','plugins/automocion/');
      if($p) {
   ...
   public function url()
   {
      if( !is_null($this->idvehiculo) ) {
         return "index.php?page=automocion_vehiculo_prov&id=".$this->idvehiculo;
   ...
   {
      $sql = 'DELETE FROM '.$this->table_name.' WHERE idpedido IS NULL AND idalbaran IS NULL AND idfactura IS NULL;';
      return $this->db->exec($sql);
   }
   
   /**
    * Devuelve todos los registros de automocion_vehiculo_documentosp con id=$id
    * 
    * @param type $id
    * @return boolean|\self
    */
   ...
   /**
    * Devuelve el registro de automocion_vehiculo_documentosp si existe,
    * sino devuelve false
    * 
    * @return boolean
    */
   ...
   public function save()
   {
      if ($this->exists()) {
         ...
         $sql = 'INSERT INTO '.$this->table_name.
              ' (idvehiculo, idpedido, idalbaran, idfactura, kilometraje) '
            .'VALUES ('
   ...
   /**
    * Elimina de la tabla automocion_vehiculo_documentosp el registro con id=$id
    * 
    * @return type
    */
   ...
   /**
    * Elimina de la tabla automocion_vehiculo_documentosp 
   ...
   /**
    * Devuelve todos los registros de automocion_vehiculo_documentosp con idpedido=$id
    * 
    * @param type $id
    * @return \vehiculo_documento_prov
    */
   public function get_from_pedido($id)
   {
      $list = array();
      /* Si se esta recargando la página, es posible que se haya realizado una aprobación */
      $data = $this->db->select('SELECT * FROM pedidosprov WHERE idpedido = '.$this->var2str($id).';');
      if( $data ) {
      ...
      if($data) {
         foreach($data as $d) {
            $list[] = new vehiculo_documento_prov($d);
         }
      }
      return $list;
   }
   ...
   /**
    * Devuelve todos los registros de automocion_vehiculo_documentosp con idalbaran=$id
    * 
    * @param type $id
    * @param type $p
    * @return \vehiculo_documento_prov
   ...
   public function get_from_albaran($id, $p = FALSE)
   {
      $list = array();
      /* Si se esta recargando la página, es posible que se haya realizado una aprobación */
      $data = $this->db->select('SELECT * FROM albaranesprov WHERE idalbaran = '.$this->var2str($id).';');
      if( $data ) {
   ...
   /**
    * Devuelve todos los registros de automocion_vehiculo_documentosp con idfactura=$id
    * 
    * @param type $id
    * @return \vehiculo_documento_prov
    */
   ...
   /**
    * Devuelve todos los registros de automocion_vehiculo_documentosp con idvehiculo =$id
    * 
    * @param type $id
    * @return \vehiculo_documento_prov
    */
   ...
   /**
    * Devuelve todos los campos de automocion_vehiculo_documentosp con idvehiculo =$id
    * 
    * @param type $id
    * @return \vehiculo_documento_prov
    */
   ...
   /**
    * Devuelve todos los registros de automocion_vehiculo_documentosp
    * 
    * @return \vehiculo_documento_prov
    */
   public function all()
